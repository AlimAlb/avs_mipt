...
long long to_number(long long cell_addr){
    if (MEMORY[cell_addr] / 0x10000000000000 / 8  == 0){
        return MEMORY[cell_addr];
    }
    else{
        long long tmp = ~(MEMORY[cell_addr]-1) & 0x00FFFFFFFFFFFFFF;
        tmp = -tmp;
        return tmp;
    }
}
...
void add(int addr1, int addr2, int addr3){
    ...
    printf("%.4x -- %lld + %lld\n", addr3, to_number(addr1), to_number(addr2));
}
...
void mult(int addr1, int addr2, int addr3){
    printf("addr1 - %x, value - %lld,  addr2 - %x, value - %lld\n", addr1, MEMORY[addr1], addr2, MEMORY[addr2]);

    MEMORY[addr3] = to_number(addr1) * to_number(addr2);
    printf("%.4x -- %lld * %lld = %lld\n", addr3, to_number(addr1), to_number(addr2), MEMORY[addr3]);
}

void division(int addr1, int addr2, int addr3){
    printf("addr1 - %x, value - %llx,  addr2 - %x, value - %lld\n", addr1, MEMORY[addr1], addr2, MEMORY[addr2]);
    printf("%lld\n", ~(MEMORY[addr1]-1));
    MEMORY[addr3] =to_number(addr1) / to_number(addr2);
    MEMORY[addr3 + 1] = to_number(addr1) % to_number(addr2);
    printf("%.4x -- %lld / %lld\n", addr3, to_number(addr1), to_number(addr2));
}
...
int run_operation(int* opers,int curr_cell){
    switch (opers[0])
    {
        ...
        case 0x81: return opers[1] == opers[2] ? opers[3] : curr_cell+1;
        case 0x82: return opers[1] != opers[2] ? opers[3] : curr_cell+1;
        case 0x83: return opers[1] < opers[2] ? opers[3] : curr_cell+1;
        case 0x84: return opers[1] >= opers[2] ? opers[3] : curr_cell+1;
        case 0x85: return opers[1] <= opers[2] ? opers[3] : curr_cell+1;
        case 0x86: return opers[1] > opers[2] ? opers[3] : curr_cell+1;

        case 0x00: MEMORY[opers[3]] = MEMORY[opers[1]]; return curr_cell+1;

        case 0x99: return sizeof(MEMORY)/8;
    }
}
